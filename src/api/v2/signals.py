...
- Analyse et génération de signaux en direct
- Enregistrement automatique dans la table d'historique
- Diffusion automatique des notifications push aux utilisateurs abonnés
"""
...
async def analyze_asset(
    symbol: str,
    main_tf: Optional[str] = Query(default=MAIN_TIMEFRAME, description="Timeframe principal (ex: 1h)"),
    confirm_tf: Optional[str] = Query(default=CONFIRMATION_TIMEFRAME, description="Timeframe confirmation (ex: 4h)"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """
    Lance une analyse complète d'un actif (EUR/USD, GBP/USD, USD/JPY, XAU/USD).

    Si un signal BUY ou SELL avec confiance >= 70% est généré :
    - Il est enregistré en base de données pour l'historique
    - Une notification push est envoyée aux utilisateurs concernés
    """
    # 1. Génération du signal via le moteur déterministe + IA
    ...
    # 3. Diffusion Push si signal exploitable (BUY ou SELL)
...
def get_signal_history(
    ...
    """Retourne l'historique des signaux récents."""
